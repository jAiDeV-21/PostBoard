...
async def update_post_full(post_id: int, post_data: PostCreate, db: db_dependency):
    result = await db.execute(
...
async def update_post_partial(post_id: int, post_data: PostUpdate, db: db_dependency):
    result = await db.execute(
        select(models.Post).where(models.Post.id == post_id),
    )
    post = result.scalars().first()
